...
(function(){
  ...
  const TOPICS = (window.SPEAKING_TOPICS || [])
    .concat(window.SPEAKING_TOPICS_EXTRA || [])
    .concat(window.SPEAKING_TOPICS_OPIC || []);   // OPIc 63분류 보강분
  ...
  function updateProgress(){
    ...
    $("foot-p6").textContent=(window.PART6||[]).reduce((n,p)=>n+p.qs.length,0);
    $("foot-p7").textContent=(window.PART7||[]).concat(window.PART7_EXTRA||[],window.PART7_SINGLE||[]).reduce((n,p)=>n+p.qs.length,0);
    ...
    $("foot-det").textContent=((d.readComplete||[]).length+(d.listenType||[]).length+
      ...
      (dx.fillBlanks||[]).length+irQ+ilQ+(dx.writeSpeakExtra||[]).length+
  ...
  const WIDE_PANELS = ["speak","write","read","gram","struct","slang","det","info","topic","ptype","opic","p6"];
  function enterPanel(tab){
    ...
    if(tab==="p6"){
      if(!$("p6-body").innerHTML){ p6Levels(); startP6(); }
      else if(p6Mode==="solve" && p6Ses && p6Ses.answered){ $("p6-next") && $("p6-next").click(); }
    }
  ...
  const NAV=[
    ...
    {id:"toeic", icon:"💼", name:"TOEIC",
     ...
     tabs:[
       ...
       {id:"toeic-p7",    label:"🗂 Part 7 독해",    panel:"topic", pre:()=>chipPre("topic-exam",'[data-tex="P7"]')},
