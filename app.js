  ...
  function renderDetWS(){
    ...
    const draw=f=>{
      ...
      $("det-ws-list").querySelectorAll(".sp-item").forEach(item=>{
        ...
        item.querySelector(".sp-head").addEventListener("click",()=>{ const o=item.classList.toggle("open"); if(!o)stopSpeak(); });
        item.querySelector('[data-act="play"]').addEventListener("click",e=>{ e.stopPropagation(); const b=e.currentTarget; b.textContent="🔊 재생 중…"; speak(t.answerEn.replace(/\[[^\]]*\]/g,""),{rate:.92,onend:()=>b.textContent="▶ 모범답변 듣기"}); });
    ...
    function bind(box,slice){
      qsa(".sp-item",box).forEach(item=>{
        ...
        playBtn.addEventListener("click",e=>{
          ...
          speak(t.answerEn,{rate:.92,onend:()=>{ playBtn.textContent="▶ "+PLAY; }});
        });
